...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--in", dest="in_dir", required=True, help="Cartella del modello HF (merge SFT/ORPO).")
    p.add_argument("--out", dest="out_path", required=True, help="Percorso del file GGUF quantizzato di output.")
...
def _manual_instructions(in_dir: str, out_path: str, quant: str) -> None:
    ...
    print("cmake -B llama.cpp/build -S llama.cpp && cmake --build llama.cpp/build -j --config Release")
...
def main(argv: list[str] | None = None) -> str | None:
    ...
    if not convert_script or not quant_bin:
        logger.warning(
            ...
            bool(convert_script), bool(quant_bin),
        )
    ...
    convert_cmd = [sys.executable, convert_script, args.in_dir, "--outfile", f16_path, "--outtype", "f16"]
    logger.info("Conversione HF->GGUF f16: %s", " ".join(convert_cmd))
    try:
        subprocess.run(convert_cmd, check=True)
    except subprocess.CalledProcessError as exc:
        raise SystemExit(f"Conversione fallita (exit {exc.returncode}). Vedi log llama.cpp.") from exc
