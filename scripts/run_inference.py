...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--prompt", default=None, help="Prompt utente singolo; se assente avvia la REPL.")
    p.add_argument("--system", default=SYSTEM_DEFAULT, help="System prompt (default: SYSTEM_DEFAULT italiano).")
    p.add_argument("--max-new-tokens", type=int, default=512, help="Token massimi generati (default: 512).")
    p.add_argument("--temperature", type=float, default=0.7, help="Temperatura di campionamento (default: 0.7).")
    p.add_argument("--top-p", type=float, default=0.9, help="Nucleus sampling top-p (default: 0.9).")
    p.add_argument(
        "--device",
        default="auto",
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Carico il modello '%s'%s...", args.model, f" + adapter '{args.adapter}'" if args.adapter else "")
